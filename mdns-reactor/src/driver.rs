...
impl<N: Net> DriverState<N> {
  ...
  fn handle_packet(&mut self, pkt: Packet) {
    ...
    if packet_is_response(&pkt.data) && pkt.src.port() != mdns_udp::constants::MDNS_PORT {
      tracing::debug!(
        src = %pkt.src,
    ...
    // mdns_udp::recv_with_meta); UNSPECIFIED/0 when PKTINFO is unavailable.
    ...
    let mut removed_services: Vec<ServiceHandle> = Vec::new();
    {
      ...
      for h in handles {
        ...
        if let Some(ctx) = queries.get_mut(&h) {
          if !new_answers.is_empty() || evicted_before_seen > 0 {
...
/// (58.F1). Updates are sent front-to-back in insertion order, stopping
...
enum MatchMode {
  ...
  /// [`mdns_udp::RX_TIMESTAMP_GRAIN`]) and within [`SELF_SEND_TTL`] — this
...
///   ([`mdns_udp::RX_TIMESTAMP_GRAIN`]: zero for nanosecond `SO_TIMESTAMPNS`,
...
fn reference_matches(reference: SystemTime, sent: SystemTime, mode: MatchMode) -> bool {
  match reference.duration_since(sent) {
    ...
    Err(behind) => mode == MatchMode::Ordered && behind.duration() <= mdns_udp::RX_TIMESTAMP_GRAIN,
  }
}
...
async fn driver_task<N: Net>(
  ...
  loop {
    ...
    // the PRIOR tick's transmits are matched against the R36 hash
...
async fn recv_loop<N: Net>(
  ...
  loop {
    ...
    // NOT consume the datagram), then `mdns_udp::recv_with_meta` does the
    ...
    #[cfg(unix)]
    {
      ...
      match mdns_udp::recv_with_meta(fd, &mut buf, via_v4) {
        Ok(meta) => {
          let n = meta.len();
    ...
    #[cfg(windows)]
    {
      ...
      match mdns_udp::recv_with_meta(raw, &mut buf, via_v4) {
        Ok(meta) => {
          let n = meta.len();
          let data = buf.get(..n).unwrap_or(&buf).to_vec();
          let pkt = Packet {
...
mod tests {
  ...
  fn self_send_ordered_tolerates_microsecond_truncation() {
    assert_eq!(mdns_udp::RX_TIMESTAMP_GRAIN, Duration::from_micros(1));
    let sent = SystemTime::now();
    let mut tracker = Vec::new();
    record_self_send(&mut tracker, b"trunc", sent);
    let truncated_rx = sent - (mdns_udp::RX_TIMESTAMP_GRAIN - Duration::from_nanos(1));
    assert!(take_self_send(
      &mut tracker,
      b"trunc",
      truncated_rx,
    ...
    record_self_send(&mut tracker, b"trunc", sent);
    let too_early = sent - (mdns_udp::RX_TIMESTAMP_GRAIN + Duration::from_micros(4));
    assert!(!take_self_send(
      &mut tracker,
      b"trunc",
      too_early,
  ...
  fn self_send_ordered_nanosecond_rejects_pre_send() {
    assert_eq!(mdns_udp::RX_TIMESTAMP_GRAIN, Duration::ZERO);
    let sent = SystemTime::now();
    let mut tracker = Vec::new();
    record_self_send(&mut tracker, b"probe", sent);
