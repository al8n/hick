...
Used by `mdns-reactor`, `mdns-compio`, and `mdns-monoio` as a shared platform layer. Each async crate creates a `std::net::UdpSocket` via this crate's helpers, then wraps it in its own runtime-native UDP type.
