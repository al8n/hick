...
/// Concrete `mdns-proto::ServiceRoute` instantiation used by the compio driver.
/// The route OWNS the `Service` state machine — which is why the service pools
/// appear in its parameters as well as the endpoint's — so the driver never
/// holds one: it observes a service through the read-only
...
/// Driver-side per-service context: the handle-owned delivery mailbox, a
/// cancellation flag, and the encode/wire bookkeeping.
///
...
/// goes through an `endpoint.*_service*` method.
///
...
pub(crate) struct ServiceCtx {
  ...
  /// Count of consecutive `poll_service_transmit` errors for this service. Reset
  ...
  /// losing the `Conflict`. Meanwhile `errored` makes every service-polling pump
  /// skip this ctx so a finished state machine can't be re-polled into a
  /// busy-spin.
...
pub(crate) struct State {
  ...
  /// One take-once credit per multicast datagram this endpoint recently
  /// transmitted, so the kernel's loopback copy is recognized as our own instead
  /// of being ingested as a peer's. Credits are recorded by
  /// [`note_multicast_attempt`], have their claim window opened by the
  /// [`SelfSendTracker::seal`] at the top of [`run`]'s loop, and are consumed by
  /// [`Self::handle_datagram`].
  /// The seal generation observed at the last park entry, so the receive path can
  /// prove the seal it relies on happened BEFORE the park rather than inside it.
  /// See [`SelfSendTracker::seal_generation`] and the check in [`run`].
  ///
  /// Unconditional here, unlike the readiness driver's, and the difference is a
  /// property of this loop rather than a choice: [`run`] has no `continue`, its
  /// only receives are the four `select!` arms, and [`Self::note_park_entry`]
  /// precedes all of them with nothing in between — so **every** datagram this
  /// driver handles came back out of a park and the ordering question is always
  /// meaningful. A loop that could reach a receive without parking would have to
  /// scope the comparison to the receives that did.
  ///
  /// Debug builds only: it feeds assertions and no decision.
  #[cfg(debug_assertions)]
  pub(crate) sealed_generation_at_park: u64,
...
impl State {
  ...
  /// and ticks — its state machine is finished), then hand the retirement to
  /// [`ProtoEndpoint::unregister_service`]. The endpoint takes the snapshot of
  /// what the CURRENT name's goodbye must retract, drains any still-pending §9
  /// rename handoff into its own detached item, KEEPS the route (holding the
  /// name against a same-name re-registration), and drives the TTL=0 goodbye
  ...
  /// detached withdrawal item the endpoint enqueued for itself when it performed
  /// the rename. A never-announced service has an empty snapshot and completes
  /// on the next loop iteration with no datagram on the wire.
  ///
  ...
  /// is GC'd. `unregister_service` is idempotent, so calling this for an
  /// already-withdrawing service is a no-op. A no-op for an unknown driver handle.
  ///
  /// # Why the snapshot is never short
  ///
  /// The snapshot the endpoint takes reports only what a confirm has latched, so
  /// it must be taken with no datagram of this service's outstanding — otherwise
  /// the goodbye omits exactly the records a still-flying datagram is about to
  /// place in peer caches, and their TTLs would only start at that late
  /// transmission.
  ///
  ...
  /// update drain's terminal teardown, and the transmit pump's encode-failure
  /// escalation alike. The only handle-side entry points that could
  ...
  /// Drive endpoint + per-query + per-service timer-based work: the endpoint
  /// cache sweep, query timeouts, and each live service's lifecycle timer.
  pub(crate) fn fire_timeouts(&mut self, now: StdInstant) {
    ...
    // Same split borrow for the service sweep: `handle_service_timeout` touches
    // only the disjoint `endpoint` field, so the map is read in place rather
    // than snapshotted into a per-tick Vec.
    let Self {
      endpoint, services, ..
    } = &mut *self;
    for (&h, ctx) in services.iter() {
      // Don't tick a withdrawn (cancelled) or structurally-dead (errored)
      // service — a finished state machine must not be driven (see
      // `ServiceCtx::errored`).
      if !ctx.cancelled && !ctx.errored {
  ...
  /// Drain pending `ServiceUpdate`s out of each registered service into the
  /// handle-owned [`ServiceMailbox`] so
  ...
  ///
  /// A [`ServiceUpdate::Renamed`] arriving here is a NOTIFICATION and nothing
  /// more: the endpoint chose a name its own route table does not hold, mirrored
  /// it into the route in the same borrow, and enqueued the old name's §10.1
  /// goodbye as its own detached withdrawal item — all inside
  /// `handle_service_timeout`. There is no collision left to reconcile.
  pub(crate) fn push_service_updates(&mut self, now: StdInstant) -> bool {
    ...
    // `&mut` access to `self.endpoint` (for `poll_service`) and
    // `self.services.get(&h)` — a single `values_mut()` borrow would lock
    // `self.endpoint` out.
    ...
    while i < self.svc_handle_scratch.len() {
      ...
      i += 1;
      // A structurally-dead service (see `ServiceCtx::errored`) is never polled —
      // it can't produce more updates. Its escalation `Conflict` already sits in
      // the handle-owned mailbox's reserved terminal slot and is drained directly
      // by `Service::next`; the wake for it is carried by the withdrawal the
      // escalation began (its deadline re-settles the driver, and the completion
      // GC notifies), so there is nothing to do here but skip it.
      if self.services.get(&h).is_some_and(|c| c.errored) {
        continue;
      }
      // Drain this service's events one at a time. Each `poll_service` returns
      // an owned `Option<ServiceUpdate>` and drops its `&mut` borrow before the
      // body runs, so the body can re-borrow `self.services` / `self.endpoint`
      // freely.
      while let Some(upd) = self.endpoint.poll_service(h) {
        if upd.is_renamed() {
          ...
          // already asserted — so without the advance here a delayed echo of the
          // abandoned owner would still read as current, adjudicate, and defeat
        ...
        // Wake on every drained update regardless of whether coalescing
        // dropped it, so a parked `Service::next` still re-checks state. This
        // matches the pre-coalescing wake semantics.
        pushed_any = true;
        // A terminal — an unresolvable §9 conflict, or the host name claimed
        // during probing — RETIRES the service: begin the endpoint-owned §10.1
        // withdrawal so the ctx/route are GC'd and the service stops serving,
        // instead of leaving a zombie live — still answering queries, with
        // `Service::next` reporting end-of-stream — until the caller drops the
        // handle. The handle-owned mailbox outlives the ctx, so the terminal
        // still reaches the host after the completion GC.
        if is_terminal {
          self.begin_service_withdrawal(h, now);
          break;
        }
  ...
  pub(crate) fn poll_one_transmit(
    ...
    while i < self.svc_handle_scratch.len() {
      ...
      //
      // NLL note: every `ctx` borrow below is scoped to its own arm so none of
      // them outlives the `self.endpoint` call that produced the result, nor
      // reaches the post-match `begin_service_withdrawal` call.
      let escalated = match self.endpoint.poll_service_transmit(h, now, scratch) {
        Ok(Some(t)) => {
        ...
        Err(_e) => {
          ...
          if ctx.encode_failures >= MAX_CONSECUTIVE_ENCODE_ERRORS {
            ...
            // remove the ctx — the mailbox outlives the ctx, so the `Conflict`
            // survives the UNCONDITIONAL GC the post-match
            ...
            // service-polling pump skips it from here on.
            warn!(
              handle = ?h,
              error = ?_e,
              scratch_size = scratch.len(),
              consecutive_failures = ctx.encode_failures,
              "Endpoint::poll_service_transmit failed; escalating to Conflict and beginning withdrawal"
            );
            ctx
              .mailbox
              .borrow_mut()
      ...
      if escalated {
        ...
        // `unregister_service` is idempotent.
        self.begin_service_withdrawal(h, now);
      }
    }
  ...
  ///
  /// The confirm is also where the endpoint mirrors the service's
  /// CONFIRMED-ADVERTISED host set into its route — so sibling host-address
  /// retention during a same-host withdrawal honours what this service ACTUALLY
  /// announced rather than its configured addresses — and where it cancels a
  /// renamed-away name's goodbye that a COMPLETE announcement of the new name
  /// has superseded. Both rest on facts only the core holds, so this driver
  /// hands over the per-family I/O outcome and nothing else.
  pub(crate) fn note_service_transmit_outcome(
    ...
    self
      .endpoint
      .note_service_transmit_outcome(h, fallback_at, v4, v6)
  }
  ...
  pub(crate) fn note_query_transmit_outcome(
    ...
    self
      .endpoint
      .note_query_transmit_outcome(h, fallback_at, v4, v6)
  }
  ...
  pub(crate) fn poll_deadline(&self) -> Option<StdInstant> {
    ...
    for (h, ctx) in &self.services {
      // A structurally-dead / withdrawing service (see `ServiceCtx::errored`)
      // must not contribute a deadline — its state machine is finished and
      // its withdrawal schedule lives in the endpoint (folded into
      // `poll_timeout` above); contributing its stale timeout would pin
      // the driver awake despite it never being polled.
      if ctx.errored {
        continue;
      }
      if let Some(t) = self.endpoint.poll_service_timeout(*h) {
  ...
  /// the call site in [`run`] for why the check belongs there and not beside
  /// `seal`.
  #[cfg(debug_assertions)]
  pub(crate) fn note_park_entry(&mut self) {
    debug_assert!(
      !self.selfsend.has_unsealed(),
      "a credit recorded this iteration is still unsealed at the park entry: \
       `seal` must run after every pump and before this park"
    );
  ...
  pub(crate) fn handle_datagram(&mut self, meta: &crate::socket::RecvMeta, rx: &RxDatagram<'_>) {
    ...
    // park entry in `run`; what this adds is that no path reaches a claim
    // with an unsealed credit, whatever route it took here.
    debug_assert!(
      !self.selfsend.has_unsealed(),
      "a credit reached the receive path unsealed: `seal` must sit between this \
       iteration's sends and this receive"
    );
    // The ordering half. The boundary recorded which seal it was relying on; if a
    // window has opened since, the seal happened inside or after the park and
    // every credit it anchored is a whole park too late.
    #[cfg(debug_assertions)]
    debug_assert_eq!(
      self.selfsend.seal_generation(),
      self.sealed_generation_at_park,
    ...
    // Self-send classification: content fingerprint plus the family this
    // datagram arrived on, take-once.
    //
    // The family is half the key. A multicast transmit fans out to both sockets
    // with IDENTICAL bytes and two separately-stamped credits, and the `select!`
    // below does not fix which socket is read first — so without it the second
    ...
    for ev in route_events {
      match ev {
        // `ToService` and `ToQuery` are both applied inside `endpoint.handle`,
        // at this datagram's receipt instant, to state machines the endpoint
        // owns; `CacheUpdated` is a hint for future cache subscribers. Any
...
pub(crate) async fn run(
  ...
  loop {
    ...
    let pump_budget_exhausted = loop {
      ...
      .await;
      // Confirm the pending transmit with the honest per-family I/O facts. The
      // core latches §10.1 goodbye ownership for whatever reached a wire and
      // advances the §8.1 probe sequence / §8.3 announce phase / §5.2 retry budget
      // only once every obligated family carried it.
      //
      ...
      // happened, so a healthy family's freshness is never backdated by however
      ...
      let confirm = {
        ...
        match origin {
          TransmitOrigin::Service(h) => {
            state.note_service_transmit_outcome(h, at, fanout.v4, fanout.v6)
          }
          TransmitOrigin::Query(h) => {
            state.note_query_transmit_outcome(h, at, fanout.v4, fanout.v6)
          }
        }
      };
      // The core weighed the refusals against this datagram's obligation and says
      // the producer cannot make progress: no bound family can ever carry these
      // bytes and the core will keep re-arming them, so the producer would
      // probe/announce (or re-question) forever with nothing on any wire. Retire
      // it on exactly the terms a persistent encode failure takes, and AFTER the
      // confirm above — which has already spent the commit token and latched
      // nothing, so the §10.1 goodbye is built from a settled service.
      if confirm.retire_producer() {
        ...
        match origin {
          TransmitOrigin::Service(h) => {
            ...
            if let Some(ctx) = state.services.get(&h) {
              ctx
                .mailbox
                .borrow_mut()
                .set_terminal(ServiceUpdate::Conflict);
            }
            state.begin_service_withdrawal(h, now);
          }
          TransmitOrigin::Query(h) => {
            warn!(
              handle = ?h,
            ...
            if let Some(ctx) = state.queries.get_mut(&h) {
              ctx.errored = true;
            }
            state.endpoint.retire_query(h);
          }
        }
        inner.notify.notify();
      }
    ...
    //     iteration, after 1b has also had a chance to begin any terminal-conflict
    //     withdrawal.
    {
      let now = StdInstant::now();
      let mut state = inner.state.borrow_mut();
    ...
    // 1b. drain pending `ServiceUpdate`s out of each registered service and into
    //     the handle-owned `ServiceMailbox` so listeners parked on
    //     `Service::next` can pop them.  The borrow is brief and is dropped
    //     before any `.await`.
    //
    //     ORDERING NOTE: this runs before the withdrawal pump (1a) below so a
    //     terminal-conflict withdrawal begun HERE (`push_service_updates` calls
    //     `begin_service_withdrawal`, whose first goodbye round is due immediately)
    //     is pumped on-wire in this SAME iteration. The endpoint holds the name
    //     for the whole withdrawal, so a same-name replacement cannot register (and
    //     evict that name from peer caches) until the goodbye completes —
    ...
    //     (1a-pre), a terminal conflict (1b above), and the transmit-pump /
    //     encode-failure escalations all begin endpoint-owned withdrawals — as does
    //     a §9 rename, whose vacated name the endpoint retracts through a detached
    //     item of its own. This pumps each due TTL=0 goodbye datagram (fanned to
    //     BOTH families) and, after draining, frees each completed route + GCs its
    //     driver ctx. Running AFTER `push_service_updates` (1b) ensures a
    //     withdrawal begun this iteration flushes its first goodbye on-wire the
    //     same iteration.
    let withdrawals_budget_stopped =
      drain_withdrawals(&inner, &sock_v4, &sock_v6, &mut scratch, &mut budget).await;

    // Open the claim window on everything the pumps above recorded: the last
    // record stage of this iteration is behind us, and the recv arms are not yet
    // armed. `SELF_SEND_TTL` runs from here, which is the first instant an echo
    // of those sends could be seen. Sealing later — in a receive arm, after the
    // park — would anchor every credit a whole park late and let a stale one
    // suppress a peer datagram arbitrarily long after the send; sealing earlier,
    // at the loop top, would leave these credits unsealed across the park, and an
    // unsealed credit is live UNCONDITIONALLY. The park entry below checks it.
    //
    // It takes no instant: the anchor is read inside the call, after the expiry
    // sweep it performs first.
    inner.state.borrow_mut().selfsend.seal();
    ...
    // 3. compute the next deadline, and decide whether to re-settle IMMEDIATELY
    ...
    // 4. arm the timer future. `Either<sleep, pending>` keeps both arms with
    ...
    // 5. arm the notify future. `LocalNotify::listen` is awaitable directly;
    //    fuse it so `select!` accepts it.
    let notify_fut = inner.notify.listen().fuse();
    futures::pin_mut!(notify_fut);

    // 6. open the self-send claim window on everything the pumps above recorded,
    //    and nowhere else in this driver.
    //
    //    **Recording and window-opening must straddle the receive**, so this sits
    //    between the last statement that can record a credit (the withdrawal pump
    //    at 1a, and the transmit pump before it) and the only thing that can claim
    //    one — the recv arms of the `select!` immediately below. Nothing between
    //    here and the park records.
    //
    //    A loop-top seal instead of this one sits on the same side of the receive
    //    as the records it is meant to open: this iteration's credits would still
    //    be unsealed when the park below returns a datagram, and an unsealed
    //    credit is live UNCONDITIONALLY. With no timer deadline the park is
    //    unbounded, so a byte-identical peer datagram arriving arbitrarily long
    //    after the send would still be swallowed as our own echo, and
    //    `SELF_SEND_TTL` would bound nothing on exactly the path it exists to
    //    bound.
    //
    //    Not at record time either: the recording iteration's own outbound time is
    //    structurally claim-free — the recv arms have not been armed yet — so
    //    charging it re-expires credits that never had a claim opportunity (a send
    //    that stalls in the kernel, a second fan-out later in the same iteration,
    //    a §10.1 goodbye a whole TTL after the announcement whose credit is still
    //    unclaimed). What this placement costs is over-retention bounded by one
    //    park, and over-retention is the CHEAP direction: a stale credit can at
    //    worst suppress one byte-identical peer datagram, and take-once bounds it
    //    to one. Under-retention is the expensive one — our own announcement
    //    ingested as peer traffic, a phantom RFC 6762 §9 conflict against
    //    ourselves and the rename that follows.
    //
    //    `seal` takes no instant and is deliberately given none: it reads the
    //    monotonic clock itself, after the expiry sweep it performs first, so the
    //    anchor cannot be a reading that sweep already spent. The borrow is scoped
    //    to this statement so it cannot collide with any borrow below.
    // THE PARK ENTRY, and the one place the seal's placement is actually checked.
    //
    // Deliberately NOT adjacent to the `seal` above: a check written straight
    // after `seal` is vacuous, since sealing is exactly what makes nothing
    // unsealed. Asked HERE it is a real question — "did the seal this iteration
    // relies on already happen, with every pump behind it?" — and the ways to get
    // the placement wrong all answer it "no": a seal at the loop top has the
    // pumps recording after it, and a seal that only runs in a receive arm (or
    // not at all) leaves this iteration's credits unsealed right here.
    //
    // The generation captured alongside is what `handle_datagram` compares
    // against, which pins the remaining case — a seal that ran BOTH here and
    // again inside the receive arm leaves nothing unsealed, so only a changed
    // generation shows that the credits were re-anchored a whole park late.
    //
    // It compiles out of release builds and influences no decision.
    #[cfg(debug_assertions)]
    inner.state.borrow_mut().note_park_entry();

    // 7. arm one recv future per bound family. The recv future borrows from
...
/// Which address family a socket serves, a datagram arrived on, or one leg of a
/// fan-out was addressed to.
///
/// `hick-udp`'s type rather than one of this crate's own, because the self-send
/// tracker this driver keys credits on lives there too — one definition, so a
/// credit recorded on this crate's IPv4 and one claimed on the tracker's cannot
/// drift apart.
pub(crate) use hick_udp::Family;

/// The largest UDP payload `family` can EVER carry — the only sound proof that a
/// refused datagram can never be sent. See
...
/// Restate one family's completed attempt in the core's I/O-world vocabulary.
///
/// Every value produced here is an OBSERVED fact, and there is no "unknown"
/// because the fan-out never confirms without one: every submitted operation is
/// awaited to its true completion first. That is the completion-driver half of
/// the readiness/completion asymmetry — [`FamilyAttempt::WouldBlock`] is simply
/// VACUOUS here rather than forbidden, since compio's cancellation is documented
/// as unreliable and abandoning a submitted send would end this driver's
/// knowledge of a datagram rather than the datagram.
///
/// `body` is what decides `permanent`: a refusal proves the bytes did not go out,
/// and only the SIZE against this family's hard UDP ceiling proves they never
/// could. The errno is deliberately not consulted — `EMSGSIZE` is equally what a
/// path-MTU refusal reports, and the next attempt may get past that. See
...
pub(crate) struct Fanout {
  pub(crate) v4: FamilyAttempt<StdInstant>,
  pub(crate) v6: FamilyAttempt<StdInstant>,
}
...
/// COMPLETION returned, and success there means the kernel owns the datagram
/// rather than that the NIC has put it on the link. See
/// [`Transmit::min_family_gap`] for where each driver's boundary sits.
///
...
enum SendAttempt {
  ...
  Answered {
    ...
    /// Wall clock, read BEFORE submission. ORDERS this send against its own
    /// loopback echo, and is never an age.
    ///
    /// Suppression weighs our echo's kernel receive stamp against this one, so
    /// `submitted_wall <= kernel_send_time <= echo_rx_time` must hold or the echo
    /// reads as a peer datagram the kernel saw before our `sendto` — the credit
    /// is refused, this host processes its own probe / announcement as peer
    /// traffic, and it raises a phantom RFC 6762 §9 conflict against itself.
    /// Reading it after the completion can push it past the kernel's rx stamp;
    /// reading it before cannot.
    submitted_wall: SystemTime,
    /// Monotonic, read BEFORE submission on the statement immediately after
    /// `submitted_wall`. Anchors the CORE's refresh schedule, and is
    /// `submitted_wall`'s partner in the [`ClockPair`] the self-send credit
    /// carries.
    ///
    /// EARLY is the safe direction for the refresh schedule: an anchor at or
    /// before the true acceptance can only understate how fresh a family's peers
    /// are, so the next refresh lands sooner than strictly needed. A late anchor
    /// would push a refresh past the records' own TTL.
    ///
    /// The ADJACENCY to `submitted_wall` is what the credit needs, and it is a
    /// contract rather than a coincidence: the nanoseconds between the two reads
    /// cancel against the pair the claim takes, so only a genuine wall-clock step
    /// can put the two elapsed times further apart than
    /// `hick_udp::selfsend::WALL_STEP_TOLERANCE`. It is not the credit's age
    /// either — that runs from the seal at the top of the next loop iteration.
...
async fn attempt_send_to<S: SendDatagram>(
  ...
  // TTL.
  //
  // And they stay ADJACENT, wall first, with nothing between them: the self-send
  // credit pairs them into a `ClockPair`, whose whole job is telling real elapsed
  // time from a wall clock that moved on its own. Anything inserted here is
  // charged to that difference, and enough of it reads as a clock step that never
  // happened — which discards this credit's ordering evidence.
...
///
/// `family` is passed rather than read off `_dst` because it is what the credit
/// is KEYED on, and the caller is the fan-out that chose which socket to offer
/// the datagram to — so the credit is keyed by the same thing that decided the
/// send, not by re-deriving it from an address afterwards.
fn note_multicast_attempt(
  ...
  match attempt {
    ...
    SendAttempt::Answered {
      ...
      let mut state = inner.state.borrow_mut();
      // BOTH pre-submit stamps, taken on consecutive statements in
      // `attempt_send_to` — which is the adjacency `ClockPair` is built on, and
      // the reason this pairs the two the send already holds instead of reading
      // a clock here. The wall half orders the echo against the send and must
      // stay pre-submit, since a stamp taken after the completion could land
      // after the echo's own kernel receive stamp; the monotonic half is what
      // says, at claim time, whether that wall stamp is still on the timeline the
      // receive stamp was taken on. Neither is an age — `SELF_SEND_TTL` runs from
      // the seal at the top of the NEXT loop iteration.
      state
        .selfsend
        .record(family, body, ClockPair::new(*submitted_wall, *submitted_at));
      #[cfg(feature = "stats")]
      {
...
async fn send_via<S: SendDatagram>(
  ...
  if is_mdns_multicast_dst(dst) {
    ...
    note_multicast_attempt(inner, Family::V4, &a4, body, MDNS_V4_DST, "transmit");
    note_multicast_attempt(inner, Family::V6, &a6, body, MDNS_V6_DST, "transmit");
    return fanout;
  }
  ...
  // `MAX_SELF_SEND_ENTRIES` a record declines the NEW entry — so a legacy-query
  // flood would starve the genuine multicast credits that suppression actually
  // depends on.
...
async fn drain_withdrawals(
  ...
  loop {
    ...
    .await;
    // The SAME restatement a positive-TTL fan-out makes: what a socket did means
    // the same thing whatever the datagram was for. What each fact then does to
    // this family's §10.1 debt is the endpoint's table and no part of this crate —
    // only an absent socket writes a debt off. A family withheld because it has
    // already paid every round it owed reports `Gated`, and the endpoint discards
    // any report for a family whose debt was already zero.
    let (v4_out, v6_out) = (
      attempt_of(Family::V4, &scratch[..len], &a4),
      attempt_of(Family::V6, &scratch[..len], &a6),
    );
    ...
    //
    // The instant is read HERE, after the awaited fan-out, and is never the one
    // this iteration polled with. What it re-arms is the §10.1 resend SCHEDULE — a
    // spacing bound on a wire, and the only thing pacing consecutive goodbyes,
    // since this fan-out is deliberately ungated. An anchor read before the
    // fan-out charges the next round everything this one spent waiting on the
    // kernel, and a round that lands near the interval leaves the next one already
    // due as it lands, collapsing §10.1's three sends into near-adjacent ones.
    {
      let now = StdInstant::now();
      inner
        .state
