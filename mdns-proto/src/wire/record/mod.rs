//! Resource records — type-specific parsers + the generic `RecordRef`
...
pub use ptr::PtrRecord;
pub use srv::SrvRecord;
#[allow(unused_imports)]
pub use txt::{TxtRecord, TxtSegments};
...
impl<'a> RecordRef<'a> {
  ...
  pub fn rdata_view(&self) -> Result<RecordRdata<'a>, ParseError> {
    match self.rtype {
      ResourceType::A => Ok(RecordRdata::A(ARecord::try_from_rdata(self.rdata())?)),
      ResourceType::Aaaa => Ok(RecordRdata::Aaaa(AaaaRecord::try_from_rdata(self.rdata())?)),
      ResourceType::Ptr => Ok(RecordRdata::Ptr(PtrRecord::try_from_message(
        self.message,
        self.rdata_start,
        self.rdata_len,
      )?)),
      ResourceType::Cname => Ok(RecordRdata::Cname(CnameRecord::try_from_message(
        self.message,
        self.rdata_start,
        self.rdata_len,
      )?)),
      ResourceType::Srv => Ok(RecordRdata::Srv(SrvRecord::try_from_message(
        self.message,
        self.rdata_start,
        self.rdata_len,
      )?)),
      ResourceType::Txt => Ok(RecordRdata::Txt(TxtRecord::from_rdata(self.rdata()))),
      ResourceType::Nsec => Ok(RecordRdata::Nsec(NsecRecord::try_from_message(
        self.message,
        self.rdata_start,
        self.rdata_len,
      )?)),
      _ => Ok(RecordRdata::Other(self.rdata())),
    }
  }
  ...
  pub(crate) fn canonical_rdata(&self) -> Result<alloc::vec::Vec<u8>, ParseError> {
  ...
  /// there.
  #[cfg(any(feature = "alloc", feature = "std"))]
  pub(crate) fn canonical_rdata_folded(&self) -> Result<alloc::vec::Vec<u8>, ParseError> {
    self.canonical_rdata_inner(true)
  }

  #[cfg(any(feature = "alloc", feature = "std"))]
  fn canonical_rdata_inner(&self, fold_case: bool) -> Result<alloc::vec::Vec<u8>, ParseError> {
...
mod tests {
  ...
  /// RDLENGTH · [35..] rdata.
  fn message_with_pointered_record(rtype: u16, rdata: &[u8]) -> alloc::vec::Vec<u8> {
  ...
  fn canonical_rdata_expands_srv_target() {
    ...
    let (rec, _) = RecordRef::try_parse(&msg, 23).unwrap();
    let out = rec.canonical_rdata().unwrap();
    let mut expected = alloc::vec::Vec::from(&[0u8, 10, 0, 20, 0x1F, 0x90][..]);
  ...
  fn canonical_rdata_expands_cname_target() {
    ...
    assert_eq!(
      ...
      alloc::vec::Vec::from(SVC_LOCAL_WIRE),
  ...
  fn canonical_rdata_expands_nsec_next_name() {
    ...
    let (rec, _) = RecordRef::try_parse(&msg, 23).unwrap();
    let out = rec.canonical_rdata().unwrap();
    let mut expected = alloc::vec::Vec::from(SVC_LOCAL_WIRE);
  ...
  fn canonical_rdata_folds_case_but_preserved_form_does_not() {
    ...
    let mut preserved_expected = alloc::vec::Vec::from(&[4u8, b'I', b'n', b'S', b't'][..]);
    ...
    let mut folded_expected = alloc::vec::Vec::from(&[4u8, b'i', b'n', b's', b't'][..]);
  ...
  fn canonical_rdata_rejects_unhandled_name_bearing_type() {
    ...
    let (rec2, _) = RecordRef::try_parse(&msg2, 23).unwrap();
    assert_eq!(
      rec2.canonical_rdata().unwrap(),
      alloc::vec::Vec::from(&opaque[..])
    );
    ...
    let (rec3, _) = RecordRef::try_parse(&msg3, 23).unwrap();
    assert!(matches!(
