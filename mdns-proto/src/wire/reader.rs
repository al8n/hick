...
use super::{HEADER_SIZE, Header, QuestionRef, RecordRef};
...
/// Iterator over `RecordRef`s in a non-question section.
...
fn skip_records(message: &[u8], mut cursor: usize, mut count: u16) -> Option<usize> {
  while count > 0 {
    let (_, next) = RecordRef::try_parse(message, cursor).ok()?;
    cursor = next;
...
impl<'a> Iterator for Records<'a> {
  type Item = Result<RecordRef<'a>, ParseError>;

  fn next(&mut self) -> Option<Self::Item> {
    if self.remaining == 0 {
      return None;
    }
    match RecordRef::try_parse(self.message, self.cursor) {
      Ok((r, next)) => {
        self.cursor = next;
        self.remaining = self.remaining.saturating_sub(1);
        Some(Ok(r))
...
mod tests {
  ...
  fn header(qd: u16, an: u16, ns: u16, ar: u16) -> alloc::vec::Vec<u8> {
    let mut h = alloc::vec![0u8; HEADER_SIZE];
