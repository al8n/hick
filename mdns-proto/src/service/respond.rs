...
use crate::{
  ...
  wire::{
    DEFAULT_COMPRESSION_TABLE, Header, MessageBuilder, RecordRdata, ResourceClass, ResourceType,
  },
};
...
pub(crate) fn canonical_rdata_for_hash<'s>(
  view: &RecordRdata<'_>,
  scratch: &'s mut alloc::vec::Vec<u8>,
...
pub(crate) fn write_probe(records: &ServiceRecords, out: &mut [u8]) -> Result<usize, EncodeError> {
  ...
  let txt: alloc::vec::Vec<alloc::vec::Vec<u8>> =
...
pub(crate) fn write_goodbye(
  ...
  if include_txt {
    let txt: alloc::vec::Vec<alloc::vec::Vec<u8>> =
      records.txt_segments().map(|s| s.to_vec()).collect();
    b.push_txt_answer(records.instance(), 0, &txt, true)?;
  }
...
/// (F3). Known-answer suppression (§7.1) can omit ANY subset
...
pub(crate) struct EmittedRecords {
  ...
  pub a: alloc::vec::Vec<Ipv4Addr>,
  /// The host AAAA addresses actually emitted.
  pub aaaa: alloc::vec::Vec<Ipv6Addr>,
...
mod tests {
  ...
  use crate::wire::{ARecord, AaaaRecord, PtrRecord, RecordRdata, SrvRecord, TxtRecord};

  #[test]
  fn canonical_a_is_4_bytes() {
    let a = ARecord::try_from_rdata(&[192, 168, 1, 10]).unwrap();
    let mut scratch = alloc::vec::Vec::new();
    let out = canonical_rdata_for_hash(&RecordRdata::A(a), &mut scratch).unwrap();
  ...
  fn write_announce_filtered_reports_emitted_groups() {
    ...
    // Suppress only A/AAAA → instance records emitted, no host address (R64 case).
  ...
  fn canonical_aaaa_is_16_bytes() {
    ...
    let rec = AaaaRecord::try_from_rdata(&rdata).unwrap();
    let mut scratch = alloc::vec::Vec::new();
    let out = canonical_rdata_for_hash(&RecordRdata::Aaaa(rec), &mut scratch).unwrap();
  ...
  fn canonical_txt_roundtrips_wire_form() {
    ...
    let txt = TxtRecord::from_rdata(raw);
    let mut scratch = alloc::vec::Vec::new();
    let out = canonical_rdata_for_hash(&RecordRdata::Txt(txt), &mut scratch).unwrap();
  ...
  fn canonical_txt_malformed_segment_returns_err() {
    ...
    let mut scratch = alloc::vec::Vec::new();
    assert!(
      canonical_rdata_for_hash(&RecordRdata::Txt(txt), &mut scratch).is_err(),
  ...
  fn canonical_ptr_is_lowercase_dotted_labels() {
    ...
    // as uncompressed length-prefixed labels so PtrRecord can parse it.
    ...
    let ptr = PtrRecord::try_from_message(&msg, 0, rdata_len).unwrap();
    let mut scratch = alloc::vec::Vec::new();
    let out = canonical_rdata_for_hash(&RecordRdata::Ptr(ptr), &mut scratch).unwrap();
  ...
  fn canonical_ptr_forward_pointer_returns_err() {
    ...
    let msg: alloc::vec::Vec<u8> = alloc::vec![0xC0u8, 0x00];
    let ptr = PtrRecord::try_from_message(&msg, 0, msg.len()).unwrap();
    let mut scratch = alloc::vec::Vec::new();
    assert!(
      canonical_rdata_for_hash(&RecordRdata::Ptr(ptr), &mut scratch).is_err(),
  ...
  fn canonical_srv_starts_with_priority_weight_port() {
    ...
    msg.push(0u8); // root
    let rdata_len = msg.len();
    let srv = SrvRecord::try_from_message(&msg, 0, rdata_len).unwrap();
    let mut scratch = alloc::vec::Vec::new();
    let out = canonical_rdata_for_hash(&RecordRdata::Srv(srv), &mut scratch).unwrap();
  ...
  fn dotted(nr: &crate::wire::NameRef<'_>) -> alloc::string::String {
  ...
  fn write_announce_emits_instance_nsec_negative_response() {
    use crate::{
      Name,
      records::ServiceRecords,
      wire::{MessageReader, RecordRdata, ResourceType},
    };
    use core::net::Ipv4Addr;
    ...
    let RecordRdata::Nsec(nsec) = r.rdata_view().unwrap() else {
  ...
  fn write_announce_filtered_emits_instance_nsec_only() {
    use crate::{
      Name,
      records::ServiceRecords,
      wire::{MessageReader, RecordRdata},
    };
  ...
  fn nsec_omitted_when_it_does_not_fit_but_answers_still_send() {
    ...
    let cut = n_full - 8;
    let mut small = alloc::vec![0u8; cut];
