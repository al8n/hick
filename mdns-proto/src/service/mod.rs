...
struct PeerProbe {
  ...
  records: alloc::vec::Vec<PeerRecord>,
}
...
#[cfg(any(feature = "alloc", feature = "std"))]
fn write_canonical_wire_name(name_str: &str, out: &mut alloc::vec::Vec<u8>) {
...
#[cfg(any(feature = "alloc", feature = "std"))]
fn rename_with_suffix(current: &str, attempt: u32) -> alloc::string::String {
...
fn compare_rr_sets_we_lose(
  ...
  let mut our_set: alloc::vec::Vec<alloc::vec::Vec<u8>> = alloc::vec::Vec::new();
  ...
  our_set.sort();
  let our_concat: alloc::vec::Vec<u8> = our_set.into_iter().flatten().collect();

  // For EACH peer bucket: if that peer's sorted set >= ours, we lose.
  for probe in peer_probes {
    let mut peer_set: alloc::vec::Vec<alloc::vec::Vec<u8>> = probe
      .records
      .iter()
      .map(|p| {
        let mut buf = alloc::vec::Vec::new();
      ...
      .collect();
    peer_set.sort();
    let peer_concat: alloc::vec::Vec<u8> = peer_set.into_iter().flatten().collect();
...
/// host-owned address (A/AAAA) independently (F3) — matching what
...
/// group the per-group latch mis-attributed (r64 → R67).
#[cfg(any(feature = "alloc", feature = "std"))]
...
pub struct Service<I, TQ, EV> {
  ...
  questioner_srcs: alloc::vec::Vec<core::net::SocketAddr>,
  ...
  peer_probes: alloc::vec::Vec<PeerProbe>,
  ...
  pending_legacy: alloc::vec::Vec<LegacyResp>,
  ...
  meta_questioner_srcs: alloc::vec::Vec<core::net::SocketAddr>,
  ...
  EV: Pool<ServiceUpdate>,
{
  ...
  pub fn note_transmit_result(&mut self, now: I, delivered: bool) {
    ...
    match kind {
      ...
      AwaitingConfirm::Announcement(emitted) => {
        ...
        // a response (R67): a full announcement emits all of
  ...
  pub fn encode_goodbye(
    ...
    // Of the addresses we advertised, drop any a sibling same-host service still
    // advertises (per-address diff).
    let a_withdraw: alloc::vec::Vec<_> = self
      .goodbye
      .a
      .iter()
      .copied()
      .filter(|ip| !retained_host_addrs.contains(&core::net::IpAddr::V4(*ip)))
      .collect();
    let aaaa_withdraw: alloc::vec::Vec<_> = self
      .goodbye
      .aaaa
      .iter()
      .copied()
      .filter(|ip| !retained_host_addrs.contains(&core::net::IpAddr::V6(*ip)))
      .collect();
    if !include_ptr
      && !include_srv
      && !include_txt
  ...
  fn our_canonical_record_for(&self, rtype: crate::wire::ResourceType) -> alloc::vec::Vec<u8> {
  ...
  fn host_record_is_ours(&self, record: &crate::wire::RecordRef<'_>) -> bool {
    match record.rdata_view() {
      Ok(crate::wire::RecordRdata::A(a)) => {
        let addr = a.addr();
        !addr.is_link_local() && self.records.a_addrs_slice().contains(&addr)
      }
      Ok(crate::wire::RecordRdata::Aaaa(a)) => {
        let addr = a.addr();
    ...
    match (self.state, event) {
      ...
      (
        ...
        if let Some(last) = self.last_conflict_reprobe {
          if let Some(elapsed) = now.checked_duration_since(last) {
            if elapsed < CONFLICT_REPROBE_MIN_INTERVAL {
              return;
            }
          }
        }
        ...
        if crate::endpoint::is_meta_query_name(sq.question().qname()) {
          if src.port() != crate::constants::MDNS_PORT {
            if self.pending_legacy.len() < MAX_LEGACY_RESPONSES {
              if let Ok(meta) = crate::Name::try_from_str(crate::endpoint::DNS_SD_META_QUERY_NAME) {
      ...
      (_, ServiceEvent::KnownAnswer(ka)) => {
        ...
        if crate::endpoint::is_meta_query_name(ka.record().name()) {
          ...
          {
            if let Ok(crate::wire::RecordRdata::Ptr(p)) = ka.record().rdata_view() {
              if crate::endpoint::names_match(self.records.service_type(), p.target()) {
    ...
    if self.tiebreak_pending && matches!(self.state, ServiceState::Init | ServiceState::Probing(_))
    {
      ...
      if we_lose {
        ...
        if self.goodbye.any_instance() {
          ...
          let owned = respond::EmittedRecords {
            ptr: self.goodbye.ptr,
            srv: self.goodbye.srv,
            txt: self.goodbye.txt,
            subtypes: self.goodbye.subtypes,
            a: alloc::vec::Vec::new(),
  ...
  pub fn poll_transmit(
          ...
          if let Some((_, remaining, _)) = self.pending_rename_goodbye.as_mut() {
            *remaining = remaining.saturating_sub(1);
            if *remaining == 0 {
              ...
              // Space the next resend; if the clock would overflow, stop here.
    ...
    if self.meta_response_deadline.is_some_and(|due| now >= due) {
      ...
      if !suppressed {
        if let Ok(meta) = crate::Name::try_from_str(crate::endpoint::DNS_SD_META_QUERY_NAME) {
          if let Ok(n) = respond::write_meta_response(&self.records, &meta, buf) {
            return Ok(Some(Transmit::new(respond::multicast_dst(), None, n)));
          }
        }
      }
    ...
    self.awaiting_confirm = match kind {
      ...
      Some(PendingTransmitKind::Announcement) => {
        ...
        // for exactly that record set, same path as a response (R67).
        Some(AwaitingConfirm::Announcement(respond::EmittedRecords {
          ptr: true,
          srv: true,
          txt: true,
          subtypes: !self.records.subtype_names().is_empty(),
          a: self.records.a_addrs_slice().to_vec(),
          aaaa: self.records.aaaa_addrs_slice().to_vec(),
        }))
      }
