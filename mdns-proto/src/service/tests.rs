...
// ── The late wire fact (a send the driver could not cancel) ────────────────

/// Everything a lifecycle round settles, snapshotted so a late wire fact can be
/// asserted to move NONE of it.
#[derive(Debug, Clone, PartialEq)]
struct LifecycleShape {
  state: ServiceState,
  lifecycle_deadline: Option<FakeInstant>,
  response_deadline: Option<FakeInstant>,
  probe_count: u8,
  announce_count: u8,
  partial_rounds: [crate::service::FamilyPatience; 2],
  partial_announce_streak: u8,
  last_delivered: [Option<FakeInstant>; 2],
  fully_announced: bool,
  awaiting_confirm: bool,
}

fn lifecycle_shape(
  svc: &Service<FakeInstant, slab::Slab<Transmit>, slab::Slab<ServiceUpdate>>,
) -> LifecycleShape {
  LifecycleShape {
    state: svc.state(),
    lifecycle_deadline: svc.lifecycle_deadline,
    response_deadline: svc.response_deadline,
    probe_count: svc.probe_count,
    announce_count: svc.announce_count,
    partial_rounds: svc.partial_rounds,
    partial_announce_streak: svc.partial_announce_streak,
    last_delivered: svc.last_delivered,
    fully_announced: svc.has_fully_announced().get(),
    awaiting_confirm: svc.awaiting_confirm.is_some(),
  }
}

/// A completion-based transport cannot definitively cancel a submitted send, so
/// a driver may learn only later that a datagram it reported `Missed` did reach a
/// wire. The two consumers of that confirm have opposite tolerances: the phase
/// and the schedule need it ON TIME and survive a conservative falsehood, while
/// goodbye ownership needs the TRUTH and survives lateness.
///
/// So the late intake is the wire half and ONLY the wire half. Ownership latches
/// — those records really are in a peer cache and §10.1 must retract them — and
/// nothing about the round that was already classified moves.
#[test]
fn a_late_wire_delivery_latches_ownership_and_moves_no_lifecycle_state() {
  let mut svc = make_service(120);
  let now = drive_to_announcing_zero(&mut svc);
  ...
  let facts = svc
    .late_wire_facts()
    .expect("a live commit token always has wire facts to capture");
  assert_eq!(
    facts.instance().as_str(),
    svc.name().as_str(),
    "the capture names the instance the datagram advertised"
  );

  // The round is confirmed ON TIME with the conservative answer: nothing was
  // observed to reach a wire.
  svc.note_transmit_outcome(at, TransmitDelivery::NONE);
  assert!(
    !svc.advertises_instance() && !svc.advertises_host(),
    "an all-missed round latches nothing"
  );
  let before = lifecycle_shape(&svc);

  // …and only afterwards does the driver learn the datagram DID go out.
  svc.note_late_wire_delivery(facts);

  assert!(
    svc.advertises_instance(),
    "the records reached a peer cache, so §10.1 must be able to retract them"
  );
  assert!(
    svc.advertises_host(),
    "…including the host addresses the announcement carried"
  );
  assert_eq!(
    lifecycle_shape(&svc),
    before,
    "a late wire fact moves NO lifecycle state: not the phase, not a deadline, \
     not the per-family patience or coverage, not the §8.3 ladder, and not the \
     all-delivered announcement proof"
  );
}

/// Idempotence matters because a dual-stack fan-out detaches up to two operations
/// for one datagram, and either may land first, both may land, or neither.
#[test]
fn a_repeated_late_wire_delivery_changes_nothing() {
  let mut svc = make_service(120);
  let now = drive_to_announcing_zero(&mut svc);
  let at = emit_announcement(&mut svc, now);
  let facts = svc.late_wire_facts().expect("a token is live");
  svc.note_transmit_outcome(at, TransmitDelivery::NONE);

  svc.note_late_wire_delivery(facts.clone());
  let once = (
    svc.goodbye.clone(),
    lifecycle_shape(&svc),
    svc.rename_goodbye_handoff.is_some(),
  );
  svc.note_late_wire_delivery(facts);
  assert_eq!(
    format!("{:?}", svc.goodbye),
    format!("{:?}", once.0),
    "latching the same records twice is a no-op"
  );
  assert_eq!(lifecycle_shape(&svc), once.1);
  assert_eq!(svc.rename_goodbye_handoff.is_some(), once.2);
}

/// The facts name the service that minted them, so a driver that mixed up two
/// unresolved operations cannot latch one service's exposure onto another —
/// which would make a goodbye withdraw records that service never emitted.
#[test]
fn late_wire_facts_from_another_service_are_a_no_op() {
  let mut a = make_service(120);
  // A DIFFERENT handle — that is the whole point of the check.
  let mut b: Service<FakeInstant, slab::Slab<Transmit>, slab::Slab<ServiceUpdate>> =
    Service::try_new(
      ServiceHandle::from_raw(1),
      make_records(120),
      FakeInstant::zero(),
      [0u8; 32],
      true,
    );
  let now = drive_to_announcing_zero(&mut a);
  let at = emit_announcement(&mut a, now);
  let facts = a.late_wire_facts().expect("a token is live");
  a.note_transmit_outcome(at, TransmitDelivery::NONE);

  b.note_late_wire_delivery(facts);
  assert!(
    !b.advertises_instance() && !b.advertises_host(),
    "facts minted by another service latch nothing here"
  );
}

/// A RFC 6762 §9 rename can happen while an operation is still unresolved. The
/// records it exposed are cached under the name the service has just left, so
/// they must go to that name's detached §10.1 goodbye — the live `goodbye` names
/// the NEW instance and would withdraw records that name never emitted.
///
/// The host name is invariant across an instance rename, so the addresses stay
/// the live service's to withdraw. This is the same split the `Stale` confirm arm
/// makes.
#[test]
fn a_late_wire_delivery_after_a_rename_goes_to_the_old_names_goodbye() {
  let mut svc = make_non_compliant_service(120);
  let now = drive_to_announcing_zero(&mut svc);
  let at = emit_announcement(&mut svc, now);
  let facts = svc.late_wire_facts().expect("a token is live");
  let old_name = facts.instance().clone();

  // Confirm on time, conservatively — the driver has observed nothing.
  svc.note_transmit_outcome(at, TransmitDelivery::NONE);
  // …then the service loses a §8.2 tiebreak and renames away.
  regress_and_rename_with_a_parked_datagram(&mut svc, at);
  assert!(
    svc.take_rename_goodbye_handoff().is_none(),
    "the old name never advertised anything yet, so the rename installs no handoff"
  );
  assert_ne!(svc.name().as_str(), old_name.as_str());
  let before = lifecycle_shape(&svc);

  // Only NOW does the operation resolve.
  svc.note_late_wire_delivery(facts);

  assert!(
    !svc.advertises_instance(),
    "the instance records belong to the name this service renamed AWAY from, so \
     the live goodbye must not claim them"
  );
  assert!(
    svc.advertises_host(),
    "the host name is invariant across an instance rename, so its addresses are \
     still ours to withdraw"
  );
  let handoff = svc
    .take_rename_goodbye_handoff()
    .expect("the old name's records are in peer caches and only this withdraws them");
  assert_eq!(
    handoff.records.instance().as_str(),
    old_name.as_str(),
    "the handoff retracts the OLD name"
  );
  assert!(
    handoff.owned.ptr() || handoff.owned.srv() || handoff.owned.txt(),
    "…and names the instance records the datagram actually emitted"
  );
  assert_eq!(
    lifecycle_shape(&svc),
    before,
    "the late intake still moves no lifecycle state, rename or not"
  );
}

/// A §8.1 probe is a QUESTION: it advertises nothing, so a late arrival latches
/// nothing either. It still names the instance, which is what a driver holding
/// the name reserved keys on.
#[test]
fn a_late_probe_delivery_latches_nothing() {
  let mut svc = make_service(120);
  let now = drive_to_probing_zero(&mut svc);
  let at = emit_probe(&mut svc, now);
  let facts = svc.late_wire_facts().expect("a probe stamps a token too");
  assert_eq!(facts.instance().as_str(), svc.name().as_str());
  svc.note_transmit_outcome(at, TransmitDelivery::NONE);
  let before = lifecycle_shape(&svc);
  ...
  assert!(
    !svc.advertises_instance() && !svc.advertises_host(),
    "a probe advertises nothing, so no arrival — on time or late — latches \
     ownership"
  );
  assert_eq!(lifecycle_shape(&svc), before);
}
