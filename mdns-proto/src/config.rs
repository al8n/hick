...
impl EndpointConfig {
  ...
  /// driver, which calls [`Endpoint::observe_send`](crate::Endpoint::observe_send)
  /// for every outgoing datagram and so does not need the lossy fallback.
  /// Single-process callers without `observe_send` integration should enable
  /// `trust_advertised_src_as_self` to recover the legacy R12/R13 behaviour.
  ...
  /// advertised A/AAAA record as a self-loopback (R12/R13 fallback).
  ///
  /// Default: `false`. The R36 [`observe_send`](crate::Endpoint::observe_send)
  /// hash cache supersedes this signal and avoids the false positives that
  /// drop legitimate same-host peer traffic. Enable only when running a
  /// single-process responder without `observe_send` integration.
  #[inline(always)]
