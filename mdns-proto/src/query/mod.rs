...
pub struct CollectedAnswer {
  ...
  rdata_key: Option<alloc::vec::Vec<u8>>,
...
mod tests {
  ...
  fn make_a_response(name: &[u8], ip: [u8; 4]) -> alloc::vec::Vec<u8> {
  ...
  fn ptr_answers_differing_only_in_case_are_deduped() {
    ...
    let mut q = make_query(ResourceType::Ptr, ResourceClass::In);
    let ptr_with_target = |first_label: &[u8]| -> alloc::vec::Vec<u8> {
      let mut msg: alloc::vec::Vec<u8> = alloc::vec::Vec::new();
      msg.extend_from_slice(&[0, 0, 0x84, 0x00, 0, 0, 0, 1, 0, 0, 0, 0]);
      msg.extend_from_slice(&[3, b's', b'v', b'c', 5, b'l', b'o', b'c', b'a', b'l', 0]);
      msg.extend_from_slice(&12u16.to_be_bytes()); // TYPE PTR
  ...
  fn cap_evicts_oldest_on_overflow() {
    ...
    let retained: alloc::vec::Vec<[u8; 4]> = q
      .answers
      .iter()
      .filter_map(|(_, a)| {
        let s = a.rdata_slice();
  ...
  struct CappedPool<const N: usize> {
    slots: alloc::vec::Vec<Option<CollectedAnswer>>,
  }

  impl<const N: usize> Pool<CollectedAnswer> for CappedPool<N> {
    type Error = CappedPoolFull;
    type Iter<'a> = alloc::boxed::Box<dyn Iterator<Item = (usize, &'a CollectedAnswer)> + 'a>;
    type IterMut<'a> =
      alloc::boxed::Box<dyn Iterator<Item = (usize, &'a mut CollectedAnswer)> + 'a>;
  ...
  fn fixed_capacity_pool_below_max_answers_evicts_and_advances_seq() {
    ...
    let retained: alloc::vec::Vec<u8> = q
      .answers
      .iter()
