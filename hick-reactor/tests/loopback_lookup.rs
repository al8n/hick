...
/// every test in this file concurrently on the same loopback link: two
/// responders probing the same name is a genuine RFC 6762 §8.2 conflict, which
/// triggers a §9 rename and leaves the loser's own querier looking up a name it
/// no longer holds. Every call site must pick a triple that's unique across
/// this file, so no two tests' responders ever contend for the same probe and
/// no test's instance can surface in another test's browse results.
async fn build_pair(
  setup_wait: Duration,
  service: &str,
  instance: &str,
...
async fn loopback_browse_resolves_service_entry() {
  ...
  let entry = match entry {
    Some(e) => e,
    None => panic!("browse did not resolve any instance of {SVC}"),
  };
